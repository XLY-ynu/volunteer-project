...
页面新增：布局管理、插播管理、操作日志、用户管理、系统管理、活动管理（签到码/统计）、志愿者注册、公共内容展示、终端预览、活动报名。
终端绑定支持选择时间段（开始/结束）；终端页可查看心跳记录和绑定记录，并按分组筛选，超时自动标记离线，可编辑属性。
内容支持关键词筛选与详情查询；媒体/内容列表分页，媒体支持下载/缩略图；公共接口提供内容/活动/终端播放查询；播放列表支持分组发布。
