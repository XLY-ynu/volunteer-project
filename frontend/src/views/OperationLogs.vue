  ...
  <div class="page-container">
    <el-card class="page-header" shadow="never">
      <div class="header-content">
        <div class="header-left">
          <h3>操作日志</h3>
          <span class="subtitle">系统操作记录，共 {{ total }} 条</span>
        </div>
    ...
    <el-card class="filter-card" shadow="never">
      <el-form :inline="true" :model="filter">
        ...
        <el-form-item label="路径">
          <el-input v-model="filter.path" placeholder="请求路径" clearable style="width: 180px" />
        </el-form-item>
      ...
      <el-table :data="list" stripe>
        <el-table-column prop="username" label="用户" width="120">
          <template #default="scope">
            <el-tag size="small">{{ scope.row.username || '匿名' }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="status" label="状态" width="80">
          <template #default="scope">
            <el-tag :type="scope.row.status >= 200 && scope.row.status < 300 ? 'success' : 'danger'" size="small">
              {{ scope.row.status }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="showCleanDialog" title="清理日志" width="400px">
      <el-alert type="warning" :closable="false" style="margin-bottom: 16px">
        <template #title>此操作不可恢复，请谨慎操作</template>
      </el-alert>
      <el-form label-width="100px">
        <el-form-item label="保留天数">
          <el-input-number v-model="cleanDays" :min="1" :max="365" />
          <span style="margin-left: 8px; color: #909399">天前的日志将被删除</span>
        </el-form-item>
      </el-form>
      <template #footer>
...
const resetFilter = () => {
  filter.value = { username: '', method: '', path: '', dateRange: null };
  page.value = 1;
  load();
};
