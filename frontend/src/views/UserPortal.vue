    ...
    <section class="my-signups">
      ...
      <div v-else class="portal-profile">
        <el-card class="profile-card" shadow="hover">
          ...
          <div class="security-settings" v-if="!profileEditing">
            <div class="reminder-title">安全设置</div>
            <el-button size="small" @click="changePwdDialog = true">修改密码</el-button>
            <el-button size="small" @click="openResetDialog">重置密码</el-button>
            <el-tag size="small" type="info">账号类型：志愿者</el-tag>
          </div>
        </el-card>
      </div>
      ...
      <el-card v-if="portalLoggedIn" class="message-card" shadow="never">
        ...
        <el-table :data="portalReminderLogs" size="small">
          ...
          <el-table-column prop="status" label="状态" width="100">
            <template #default="scope">
              <el-tag size="small" :type="scope.row.status === 'sent' ? 'success' : 'danger'">
                {{ scope.row.status }}
              </el-tag>
            </template>
          </el-table-column>
...
const loadPortalReminderLogs = async () => {
  if (!portalLoggedIn.value) {
    portalReminderLogs.value = [];
    return;
  }
  try {
    const resp = await fetchPortalReminderLogs();
    portalReminderLogs.value = resp.data?.data || [];
  } catch (e) {
    portalReminderLogs.value = [];
  }
};
...
.security-settings { display: flex; align-items: center; gap: 8px; margin-top: 12px; }
.message-card { margin-top: 12px; }
.message-title { font-weight: 600; }
.message-meta { font-size: 12px; color: #909399; margin-top: 2px; }
