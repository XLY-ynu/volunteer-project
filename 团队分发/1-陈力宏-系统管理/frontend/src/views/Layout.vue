  ...
  <el-container style="min-height: 100vh">
    <el-aside width="240px" class="aside">
      ...
      <el-menu :default-active="active" class="menu" @select="onSelect">
        ...
        <el-menu-item index="/ops"><el-icon><List /></el-icon><span>操作日志</span></el-menu-item>
...
import { HomeFilled, UserFilled, List, Setting } from '@element-plus/icons-vue';
...
<style scoped>
.aside { background: linear-gradient(180deg, #1e3a5f 0%, #0f172a 100%); color: #fff; box-shadow: 2px 0 8px rgba(0, 0, 0, 0.15); }
.logo { padding: 20px 16px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); margin-bottom: 8px; }
.logo-icon { font-size: 24px; }
.logo-text { font-weight: 600; font-size: 16px; color: #fff; }
.menu { border-right: none; background: transparent; }
.menu-group { padding: 12px 20px 6px; font-size: 11px; color: rgba(255, 255, 255, 0.5); text-transform: uppercase; letter-spacing: 1px; }
:deep(.el-menu-item) { color: rgba(255, 255, 255, 0.8); margin: 2px 8px; border-radius: 8px; height: 40px; line-height: 40px; }
:deep(.el-menu-item:hover) { background: rgba(255, 255, 255, 0.1); color: #fff; }
:deep(.el-menu-item.is-active) { background: linear-gradient(90deg, #409eff 0%, #66b1ff 100%); color: #fff; }
...
.avatar { background: linear-gradient(135deg, #409eff 0%, #66b1ff 100%); color: #fff; font-weight: 600; }
